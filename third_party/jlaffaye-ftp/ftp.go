...
type dialOptions struct {
	...
	activeMode      bool          // 主动模式(PORT/EPRT)：客户端监听，服务器主动连接数据通道
}
...
// DialWithActiveMode returns a DialOption that configures the ServerConn
// to use active mode (PORT/EPRT): the client listens on a local port and
// the server initiates the data connection to it.
func DialWithActiveMode() DialOption {
	return DialOption{func(do *dialOptions) {
		do.activeMode = true
	}}
}
...
// openActiveDataConn establishes an active-mode (PORT/EPRT) data connection:
// the client listens on a local port and announces it to the server.
// The server initiates the connection after receiving the data command
// (e.g. STOR), so the accept is deferred until the connection is used.
func (c *ServerConn) openActiveDataConn() (net.Conn, error) {
	ln, err := net.Listen("tcp", ":0")
	if err != nil {
		return nil, err
	}

	localIP := c.netConn.LocalAddr().(*net.TCPAddr).IP.String()
	port := ln.Addr().(*net.TCPAddr).Port

	if err := c.sendEPRT(localIP, port); err != nil {
		if err := c.sendPORT(localIP, port); err != nil {
			ln.Close()
			return nil, fmt.Errorf("send PORT failed: %v", err)
		}
	}

	var tlsCfg *tls.Config
	if c.options.tlsConfig != nil {
		cfg := c.options.tlsConfig.Clone()
		if len(cfg.Certificates) == 0 {
			cert, err := generateActiveModeCert()
			if err != nil {
				ln.Close()
				return nil, err
			}
	...
	return &activeDataConn{ln: ln, tlsCfg: tlsCfg, timeout: c.options.dialer.Timeout}, nil
}

// activeDataConn defers accepting the server-initiated data connection
// until the first read/write, by which time the server has connected.
...
func (a *activeDataConn) ensure() error {
	...
	if a.timeout > 0 {
		if tcpLn, ok := a.ln.(*net.TCPListener); ok {
			_ = tcpLn.SetDeadline(time.Now().Add(a.timeout))
		}
	}
	conn, err := a.ln.Accept()
	if err != nil {
		return fmt.Errorf("accept data connection failed: %v", err)
	}
	if a.tlsCfg != nil {
		conn = tls.Client(conn, a.tlsCfg)
	}
	a.accepted = conn
	return nil
}
...
func (a *activeDataConn) LocalAddr() net.Addr  { return a.ln.Addr() }
func (a *activeDataConn) RemoteAddr() net.Addr { return a.ln.Addr() }

func (a *activeDataConn) SetDeadline(t time.Time) error {
	if a.accepted != nil {
		return a.accepted.SetDeadline(t)
	}
	return nil
}
...
func (a *activeDataConn) SetReadDeadline(t time.Time) error {
	if a.accepted != nil {
		return a.accepted.SetReadDeadline(t)
	}
	return nil
}
...
func (a *activeDataConn) SetWriteDeadline(t time.Time) error {
	if a.accepted != nil {
		return a.accepted.SetWriteDeadline(t)
	}
	return nil
}

func (c *ServerConn) sendEPRT(ip string, port int) error {
	_, _, err := c.cmd(StatusCommandOK, "EPRT |1|%s|%d|", ip, port)
	return err
}

func (c *ServerConn) sendPORT(ip string, port int) error {
	_, _, err := c.cmd(StatusCommandOK, "PORT %s,%d,%d", strings.ReplaceAll(ip, ".", ","), port/256, port%256)
	return err
}

// generateActiveModeCert creates a temporary self-signed certificate used
// as the TLS server certificate on active-mode TLS data connections.
func generateActiveModeCert() (tls.Certificate, error) {
	...
	tpl := x509.Certificate{
		SerialNumber:          serial,
		Subject:               pkix.Name{CommonName: "allinssl-ftp-active-mode"},
...
func (c *ServerConn) cmdDataConnFrom(offset uint64, format string, args ...interface{}) (net.Conn, error) {
	...
	// 主动模式：150 响应意味着服务器已连接数据端口，此时完成 accept
	if act, ok := conn.(*activeDataConn); ok {
		if err := act.ensure(); err != nil {
			_ = conn.Close()
			return nil, err
		}
	}
