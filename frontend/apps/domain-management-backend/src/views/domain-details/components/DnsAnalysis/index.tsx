...
import { useDomainDetailState } from '@/views/domain-details/useStore'
...
export default defineComponent({
	...
	setup(props: DnsAnalysisProps) {
		...
		const { domainInfo } = useDomainDetailState()
		// 使用控制器
		const {
			toggleRecordStatus,
			fetchRecords,
			...
			recordCount,
			FilterForm,
		} = useDnsAnalysisController(props)
		 ...
		 */
		const handleAdd = () => {
			if (!([1, 2].indexOf(domainInfo.value?.status || 0) > -1)) return message.warning('当前域名状态异常或待注册，无法添加解析记录')
		...
		const renderEditableCell = (
			...
			if (row.record_id === '_adding_' || isEditing.value === row.record_id) {
				...
				if (field === 'record') {
					...
					return viewsOptions.value.find((item) => item.value === row.viewID)?.label ?? '-'
				} else {
					return value
				}
			}
			...
			return (
				<div class="rounded-lg my-2">
					...
					<div class="flex flex-col gap-4">
						{/* 帮助信息：在有内容时占据100%宽度 */}
						{hoveredHelpInfo ? (
							<div class="w-full" onMouseenter={handleHelpEnter} onMouseleave={handleHelpLeave}>
		...
		const DnsRecordCardList = defineComponent({
			...
			setup(cardProps) {
				const getViewLabel = (viewID?: number) =>
					viewsOptions.value.find((item) => item.value === (viewID as any))?.label ?? '-'
				return () => (
