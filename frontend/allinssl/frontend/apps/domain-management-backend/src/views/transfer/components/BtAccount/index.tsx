...
import { NFlex, NButton, NDivider, NIcon } from 'naive-ui'
import { CloseOutline, SearchOutline } from '@vicons/ionicons5'
import { useBtAccountTransferController } from './useController'
import { useApp } from '@/components/layout/useStore'

export default defineComponent({
	name: 'BtAccountTransfer',
	setup() {
		const { isMobile } = useApp()
		const { loading, TableComponent, PageComponent, FilterForm, BtAccountTransferCardList, tableData, openTransferDialog } =
			useBtAccountTransferController()
		...
		const renderList = () => (
			<>
				{isMobile.value ? (
					<NFlex vertical>
						<>
							<BtAccountTransferCardList data={tableData.value?.list || []} loading={loading.value} />
